...
/**
 *
 * @author Phil Picinic This is a singleton class that creates a single instance
 * of the Black Jack Strategy Card.
 */
public class StrategyCard {
    ...
    /**
     * Constructor creates and fills in the card
     */
    ...
    /**
     * gets an instance of the strategy card if there isn't one, one will be
     * created
     *
    ...
    /**
     * Method to get hard play value of hand
     *
     * @param myHand the player's hand
     * @param upCard the dealer's up card
     * @return the play that the basic strategy suggests
     *
     */
    public Play getBotPlay(Hand myHand, Card upCard) {
        Play play = getPlay(myHand, upCard);
        if (play == Play.SPLIT) {
            Card myCard = myHand.getCard(0);
            int handVal = myCard.value() * 2;
            if (myCard.isAce()) {
                handVal = 12;
            }
            if (myCard.isFace()) {
